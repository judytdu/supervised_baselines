...
class SupervisedBaselines:
    ...
    def _configure_logger(self, log_file):
        ...
        logging.basicConfig(
            filename=log_file, level=logging.INFO, format=log_fmt, datefmt="%H:%M:%S"
        )
    ...
    def _configure_optuna_hyperparams(self, optuna_config_dict):
        logging.info("Configuring Optuna hyperparameters")
    ...
    def _configure_neptuneai(
        self, neptune_project_name, neptune_workspace, neptune_api_token
    ):
        logging.info(
        ...
        for mt in self.model_types:
            ...
            if self.global_tuning_params is not None:
                ...
                self.optuna[mt], self.model[mt] = run_optuna.optimize_hyperparams(
                    ...
                    optimization_direction=self.global_tuning_params[
